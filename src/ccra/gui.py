...
class MainWindow(QMainWindow):
    def __init__(self,service:AppService):
        super().__init__(); self.service=service; self.current_project=None; self.setWindowTitle("Computational Chemistry Report Assistant"); self.resize(1450,900); self.setAcceptDrops(True); self._build()
    def _build(self):
        ...
        for name,widget in [("Summary",self.summary),("Files",self.files),("Raw Data",self.raw),("Drive",self.drive),("Sheets",self.sheets),("Reaction Coordinate",self.reaction_coordinate),("Energy",QTextEdit()),("Spin Density",QTextEdit()),("Geometry",QTextEdit()),("TD-DFT / Frequency",QTextEdit()),("Future Work",QTextEdit())]: self.tabs.addTab(widget,name)
        ...
        bar.addStretch(); layout.addLayout(bar); layout.addWidget(self.tabs); layout.addWidget(self.console); splitter=QSplitter(); splitter.addWidget(self.project_tree); splitter.addWidget(center); splitter.setSizes([260,1190]); self.setCentralWidget(splitter)
    ...
    def _refresh_files(self):
        ...
        if self.current_project: self.reaction_coordinate._refresh_file_combo()
